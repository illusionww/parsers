...
class LinkedinSearch(object):
    ...
    def get_persons_by_name(self, first_name, last_name):
        ...
        return [self.get_person_info_by_link(profile.find("div", "content").h3.a['href']) for profile in soup.find_all('div', 'profile-card')]
    ...
    def get_person_info_by_link(self, url):
        ...
        person = {'name': profile_overview_content.find(id="name").text,
        ...
        if experience_section:
            person['positions'] = [{'position': position.find("h4", "item-title").a.text,
                                    'organization': position.find("h5", "item-subtitle").a.text if position.find("h5", "item-subtitle").a else None,
        ...
        if skills_section:
            person['skills'] = [skill.a["title"] for skill in skills_section.ul.find_all("li", "skill") if
                                not any(x in skill["class"] for x in ["see-less", "see-more"])]
